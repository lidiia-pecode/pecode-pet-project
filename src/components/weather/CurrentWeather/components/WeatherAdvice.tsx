...
import { weatherAdviceStyles } from '../CurrentWeather.styles';
...
export default function WeatherAdvice() {
  ...
  return (
    <Box sx={{ position: 'relative' }}>
      <IconButton onClick={togglePopper} sx={weatherAdviceStyles.button}>
        <InfoOutlinedIcon />
      </IconButton>
        ...
        {({ TransitionProps }) => (
          <ClickAwayListener onClickAway={() => setAnchorEl(null)}>
            <Fade {...TransitionProps} timeout={150}>
              <Paper sx={weatherAdviceStyles.popperPaper}>
                {advice ? (
                  <Typography variant='body1'>{advice}</Typography>
                ) : (
                  <Box sx={weatherAdviceStyles.loadingBox}>
